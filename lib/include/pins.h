#pragma once

typedef uint8_t pin;

#define BUZZER_PIN 15
#define TRIG 0
#define ECHO 0

#define LASER_RECIVE 2
#define LASER_EMIT 22

#define SERVO_HOR 23
#define SERVO_VER 19

#define MOTOR_EN 4

// Forwards = straight module

#define LEFT_TOP_WHEEL_FOR 25 // IN 3
#define LEFT_TOP_WHEEL_BACK 26 // IN 4

#define RIGHT_TOP_WHEEL_FOR 27 // IN1
#define RIGHT_TOP_WHEEL_BACK 14 // IN2

#define LEFT_BOTTOM_WHEEL_FOR 32 // IN2
#define LEFT_BOTTOM_WHEEL_BACK 33 // IN1

#define RIGHT_BOTTOM_WHEEL_FOR 12 // IN 4
#define RIGHT_BOTTOM_WHEEL_BACK 13 // IN 3